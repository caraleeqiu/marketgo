...
**现场照片是信任锚点，没有合格照片整个 Skill 不成立。** 本步分四段：
开场握手 → 前置拍摄须知 → 收图 → VLM triage 四分叉。

#### 3.1.0 开场握手（能做什么 → 师傅确认 → 告诉师傅需要什么）

师傅的触发可能是一句模糊的话（「我想做一条蓝领的知识视频」/「帮我
做个视频」），也可能直接带了照片 + 描述。不管哪种，skill **第一件事**
是开场握手——先说清边界、拿到「继续」的确认，再要输入。直接带齐了
照片 + 描述的，把握手压成一句确认即可，不啰嗦重复。
...
师傅确认「继续」→ 进 §3.1.1；师傅说「不是我要的」→ 礼貌结束，不硬塞。

#### 3.1.1 前置告知拍摄须知（师傅传图前就说清楚）
...
1. 师傅在触发消息 / 对话里**已经带了**（如「我叫老王，电话 138xxxx，
   服务朝阳区」/ 附了 logo）→ **直接抽取，不重复问**。
2. 没带 → §3.6 **主动问一句**（轻量，不强制）：「要加署名卡吗？给
   名字 / 电话 / 服务区 / logo 就出你的个性化卡，不给就出通用『找本地
   水管工』卡。」师傅说不要 → `has_branding: false`，走通用兑底。
3. **partial 文字信息**：师傅只给一部分（如只有名字 + 电话，没服务区）
   → `has_branding: true`，remotion **只排师傅给的字段，不留空位 / 不
   补占位符**。
...
| 仅文字信息（无品牌图，含 partial）| 纯文字排版的个性化卡——remotion 只排有的字段 |
