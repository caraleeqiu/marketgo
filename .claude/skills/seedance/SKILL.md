...
3. **图片驱动（路径 C，v2）**：用户丢图进来想做视频。不是"平均让画面动起来"，而是**从图里识别出一个最有戏的点极致放大**。**时长由内容粒度推算（4-15s 弹性）**，不默认打满 15s；画质策略匹配图的来源感（手机/胶片/CG 等）；图的时序角色（首/中/尾/反/闪）必须显式询问。
4. **参考素材优先**：大部分好的 AI 视频不是纯文本生成的。先做首帧图、找参考视频，再写提示词。
...
**路径C — 图片驱动（Image-Driven，v2）：**
- 触发：用户上传/粘贴/拖入图片，或要求"用这张图做视频"、"图生视频提示词"、"根据图片生成视频"、"这张图怎么做成视频"
- 核心任务：**从图里识别出一个最有戏的点极致放大**，按内容粒度推算合适时长
- **必须走两轮交互（v2 升级）**：
  1. 第 1 轮：读图分析（主体/氛围/最有戏元素/**画质来源判定**/**建议时长**/比例诊断/**平台合规风险**/风险检查） → **询问图的时序角色**（首/中/尾/反/闪） → 2-3 个 Hook 候选模式
  2. 第 2 轮：用户选定模式后，输出完整可复制的 Seedance 提示词（时长 = 推算值，画质策略 = 来源对应，反应按子节拍展开）
- **可选 Step 4（CLI 执行）**：第 2 轮提示词输出后，**询问**用户是否直接调用 `dreamina` CLI 生成视频；默认不跑，用户回 `跑` / `yes` 才执行。默认 `seedance2.0_vip` + `720p` + 时长 = 推算值（非默认 15s）。详见 [cli-integration.md](references/cli-integration.md)
- **v2 四条新原则**（必须遵守）：
  - 画质即风格：视频画质必须匹配图的来源感（手机抓拍/胶片/监控/CG 等），**禁止默认电影级**
  - 时序角色显式询问：图作为首帧/中间帧/尾帧/反转帧/闪回，不能默认首帧
  - 现实物理时间常数：脸红 ≥2.5s、哭 ≥3s、惊吓 ≥1.2s 等，按子节拍展开
  - 时长服从内容：4/6/8/10/12/15s 任选，**15s 是上限不是目标**
...
**当前画面氛围**：...
**最有戏的元素**：...
**画质来源判定**：[摆拍/生活抓拍/手机日常/监控DV/胶片复古/动漫CG/抓拍偷拍] —— 一句话依据
**对应画质策略**：[关键词，如"手机随手拍质感+轻微手抖+自然曝光"]
...
- **Hook 模式**
- **图的时序角色**（首/中/尾/反/闪）
- **画质策略**（来自第 1 轮判定，必须写进风格总纲）
- **时长**（推算值，非默认 15）

**风格总纲必须包含画质来源关键词**，禁止生活类图默认电影级。**现实类反应必须按子节拍展开**（脸红 ≥2.5s、哭 ≥3s、惊吓 ≥1.2s 等）。末尾给"设计意图"说明前 2 秒生死线、核心 hook、画质真实感、时序逻辑、二刷钩子、传播预期。
...
- **平台合规风险=高 时拒跑，建议换图**（v2 新增）
- 执行后把 `submit_id` 和结果路径回显，保留审计痕迹
- 连续失败 3 次必须停下来问用户"问题是提示词还是模型"
- **post-TNS 失败积分会退回，但耗时 5-7 分钟**，提前判合规可以省时间（v2 新增）
...
- [ ] **图的时序角色已显式询问并确定**（首/中/尾/反/闪）（v2）
- [ ] **画质来源已判定**，对应画质策略已写入风格总纲（v2）
- [ ] **建议时长已推算**，并给出推算理由（非默认 15s）（v2）
- [ ] **平台合规风险已评估**（v2）
- [ ] 风险检查已扫（真人脸/文字水印/清晰度）
- [ ] 前 2 秒有视觉钩子，没有用"缓缓推进"开场
- [ ] 推算时长 X 的分镜完整，无时间空缺
- [ ] **画质来源关键词已注入风格总纲**，未误用电影级在生活类图（v2）
- [ ] **现实类反应使用了子节拍**（脸红 ≥2.5s 等）（v2）
- [ ] **图作为非首帧时**，提示词中显式写明时间锚点（v2）
